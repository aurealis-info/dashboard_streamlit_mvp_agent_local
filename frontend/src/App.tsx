...
import { DecisionDocket } from './components/DecisionDocket'
...
import type { FieldDefinition, MilestoneName, NewProjectInput, Project, ProjectStatus, ProjectUpdate, ViewMode } from './types'
...
function App() {
  ...
  const [activeNav, setActiveNav] = useState(view === 'board' ? 'lifecycle' : 'command')
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      return matchesStatus && matchesOwner && matchesFocus && (!query || haystack.includes(query))
    })
  }, [deferredSearch, focusOnly, owner, projects, status])
  ...
  const updateProject = (key: string, update: ProjectUpdate) => {
    setProjects((current) => current.map((project) => {
      ...
      if (update.currentMilestone) {
        const stageIndex = milestoneNames.indexOf(update.currentMilestone)
        next.milestones = project.milestones.map((milestone, index) => ({ ...milestone, status: index < stageIndex ? 'done' : index === stageIndex ? 'in_progress' : 'not_started' }))
        next.progress = Math.max(project.progress, Math.round((stageIndex / (milestoneNames.length - 1)) * 100))
      }
  ...
  const createProject = (input: NewProjectInput) => {
    ...
    const project: Project = {
      ...
      updatedAt: 'Just now',
      nextAction: 'Complete the initial assessment',
      nextActionDate: '2026-07-23',
      tags: ['New request'],
      stakeholders: [{ name: input.owner, role: 'Program owner', initials }],
      milestones: milestoneNames.map((name, index) => ({ name, status: index === 0 ? 'in_progress' : 'not_started', automatic: index === 0 })),
      epics: [],
  ...
  const clearFilters = () => { setSearch(''); setStatus('All'); setOwner('All'); setFocusOnly(false) }
  const showFocus = () => { setFocusOnly(true); setStatus('All'); setOwner('All'); setView('table'); setActiveNav('portfolio'); document.querySelector('.records-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }
  const resetDemo = () => {
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        <header className="topbar">
          ...
          <div className="breadcrumb"><span>Automation office</span><Icon name="chevron" size={12} /><strong>Command center</strong></div>
        ...
        <div className="workspace">
          <DecisionDocket projects={attentionProjects} onSelect={(project) => setSelectedKey(project.key)} onShowFocus={showFocus} />
          <PortfolioPulse projects={projects} />
          <section className="records-panel" aria-labelledby="portfolio-title">
            <div className="records-heading">
              <div><span className="eyebrow">Operating portfolio</span><h2 id="portfolio-title">Every initiative, one accountable view</h2><p>JIRA facts and portal-owned context stay distinct, but work together.</p></div>
              <button className="button primary" onClick={() => setProjectModal(true)}><Icon name="plus" size={16} />New initiative</button>
            </div>
            ...
            {filteredProjects.length ? (
              view === 'table'
                ...
                : <PipelineBoard projects={filteredProjects} onSelect={(project) => setSelectedKey(project.key)} onMove={(key, milestone: MilestoneName) => updateProject(key, { currentMilestone: milestone })} />
