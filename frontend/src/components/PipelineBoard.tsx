...
interface PipelineBoardProps {
  ...
  onMove: (projectKey: string, milestone: DevelopmentStage) => void
}

export function PipelineBoard({ projects, onSelect, onMove }: PipelineBoardProps) {
  const [dragging, setDragging] = useState<string | null>(null)
  const [dragOver, setDragOver] = useState<DevelopmentStage | null>(null)

  return (
    <div className="board-scroll" aria-label="Lifecycle board">
      <div className="pipeline-board">
        {developmentStages.map((milestone, index) => {
