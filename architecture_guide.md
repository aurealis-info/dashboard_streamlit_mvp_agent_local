...
- The reference UI shows all eight stages together, exposes each manual start/end value through direct calendar controls, and surfaces the same ranges inside the project register and secondary timeline. It does not automatically cascade a changed date into later stages because valid project stages may overlap; any future dependency rule must be explicit and backend-validated.
